...
ここいらあたりは有名すぎて説明する必要もないかもしれません。こ
のプレゼンを勉強会で発表したときに@michioga さんから libopenblas-dev を使った方が高速になるとのアドバイスをもらいました。
すごい高速になりました、ありがとうございます。
...
# 未知変数'u'の計算(メッシュを修正下場合)

さて、まだ時間がありますので、メッシュを変更した場合に結果がどのようになるかを確認してみたいと思います。
有限要素法においてはメッシュを細かくすると理論解により近くなることが知られています。
そこで、先程のメッシュのおおよその長さhをより小さなものにしてみます。
その結果がこちらです。
こちらから分かりますように、メッシュを細かくすると計算結果は理論解に近づくことがわかります。
